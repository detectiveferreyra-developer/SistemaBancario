$(document).ready(function(){
    $("#ingreso-detalles-nuevos-clientes").on('submit',(function(e){ // "e" IDENTIFICADOR DE ACCION PARA CAPTURAR EVENTO DE FORMULARIO
        ...
        if($FotoNitUsuarios === ""){
            AlertaUsuarioMostrar("Campo Incompleto","Por favor adjunte fotografía del número de identificación tributaria (Nit) de nuevo cliente","warning");
            return false;
        }
        if($FotoFirmaUsuarios === ""){
            AlertaUsuarioMostrar("Campo Incompleto","Por favor adjunte fotografía de la firma (procesada en máquina lectora de firmas) de nuevo cliente","warning");
            return false;
        }
        else{
        e.preventDefault(e); // "e" IDENTIFICADOR DE ACCION PARA CAPTURAR EVENTO DE FORMULARIO
        $.ajax({
            url: "../controlador/cGestionesCrediAgil.php?CrediAgilgestion=envio-datos-registro-detalles-nuevos-usuarios",
            type: "POST",
            dataType: 'json',
            ...
            if($resultado === "OK"){
                ...
                AlertaUsuarioMostrar("Registro Exitoso","Perfecto, has registrado con éxito los detalles de este nuevo usuario","success");
