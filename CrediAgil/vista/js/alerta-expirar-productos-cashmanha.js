$(document).ready(function(){  
    ...
    $(document).on('click', '#expirar-productos', function(e){
     var IDProductos = $(this).data('id'); // ID ENVIADA POR GET DESDE EL CONTROLADOR HACIA EL MODELO
     ExpirarProducto(IDProductos); // ID UNICO DE USUARIOS
     e.preventDefault();
    });
   });
   function ExpirarProducto(IDProductos){ 
    swal.fire({
        title: '¿Realmente desea expirar este producto?',
     ...
     preConfirm: function() {
       return new Promise(function() {
          $.ajax({
          url: '../controlador/cGestionesCrediAgil.php?CrediAgilgestion=expirar-productos-CrediAgil&idproducto='+IDProductos,
          type: 'POST',
          ...
          .done(function(respuesta){ // SI MODELO EJECUTA PETICION, REALIZA PETICION
            if(respuesta=="OK"){
                AlertaUsuarioMostrar("Producto Expirado","Su petición se ha completado con éxito","success");
