...
switch ($peticion_url) {
    ...
    case "nuevo-cliente":
        ...
        if ($_SESSION['id_rol'] == 1) {
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema, $IdUsuarios);
            require("../vista/Administradores/nuevo-cliente.php");
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
        } else {
            // SI EL USUARIO SE ENCUENTRA LOGUEADO, REDIRECCIONA A PAGINA PRINCIPAL DE PORTAL SEGUN SU ROL DE USUARIO ASIGNADO
            header('location:cGestionesCrediAgil.php?CrediAgilgestion=redirecciones-sistema-CrediAgil');
    ...
    case "inicioclientes":
        if ($_SESSION['id_rol'] == 5) {
            ...
            $consulta3 = $Gestiones->ConsultarAvanceCreditos_ClientesCrediAgilInterfaz($conectarsistema4, $IdUsuarios);
    ...
    case "consultar-productos-CrediAgil":
        // VISTA VALIDA SOLO PARA ADMINISTRADORES
        if ($_SESSION['id_rol'] == 1) {
            $IdUsuarios = $_SESSION['id_usuario']; // ID UNICO DE USUARIO REGISTRADO
            // CONSULTA COMPLETA DE PRODUCTOS REGISTRADOS CrediAgil [ACTIVOS]
            $consulta = $Gestiones->ConsultarProductosCrediAgil_Activos($conectarsistema);
            // CONSULTA COMPLETA DE PRODUCTOS REGISTRADOS CrediAgil [INACTIVOS]
            $consulta1 = $Gestiones->ConsultarProductosCrediAgil_Inactivos($conectarsistema1);
            // CONSULTA COMPLETA DE PRODUCTOS REGISTRADOS CrediAgil [EXPIRADOS]
            $consulta2 = $Gestiones->ConsultarProductosCrediAgil_Expirados($conectarsistema2);
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta3 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema3, $IdUsuarios);
            require('../vista/Administradores/consulta-general-productos-CrediAgil.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "consultar-productos-CrediAgil-presidencia":
        // VISTA VALIDA SOLO PARA PRESIDENCIA
        if ($_SESSION['id_rol'] == 2) {
            $IdUsuarios = $_SESSION['id_usuario']; // ID UNICO DE USUARIO REGISTRADO
            // CONSULTA COMPLETA DE PRODUCTOS REGISTRADOS CrediAgil [ACTIVOS]
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta3 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema3, $IdUsuarios);
            require('../vista/Presidencia/consulta-general-productos-CrediAgil-presidencia.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "consultar-productos-CrediAgil-gerencia":
        // VISTA VALIDA SOLO PARA GERENCIA
        if ($_SESSION['id_rol'] == 3) {
            $IdUsuarios = $_SESSION['id_usuario']; // ID UNICO DE USUARIO REGISTRADO
            // CONSULTA COMPLETA DE PRODUCTOS REGISTRADOS CrediAgil [ACTIVOS]
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta3 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema3, $IdUsuarios);
            require('../vista/Gerencia/consulta-general-productos-CrediAgil-gerencia.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "consultar-productos-CrediAgil-atencion-clientes":
        // VISTA VALIDA SOLO PARA GERENCIA
        if ($_SESSION['id_rol'] == 4) {
            $IdUsuarios = $_SESSION['id_usuario']; // ID UNICO DE USUARIO REGISTRADO
            // CONSULTA COMPLETA DE PRODUCTOS REGISTRADOS CrediAgil [ACTIVOS]
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta3 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema3, $IdUsuarios);
            require('../vista/AtencionClientes/consulta-general-productos-CrediAgil-atencion-clientes.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "activar-productos-CrediAgil":
        ...
        if ($_SESSION['id_rol'] >= 1 && $_SESSION['id_rol'] <= 3) {
            ...
            if (empty($IdProductos)) {
                ...
                $consulta = $Gestiones->ActivarProductosCrediAgil($conectarsistema, $IdProductos);
                // ENVIANDO RESPUESTA DE ACCION A MODELO
                echo json_encode($consulta);
            }
            $conectarsistema->close(); // CERRANDO CONEXION
    ...
    case "desactivar-productos-CrediAgil":
        // VISTA VALIDA PARA ADMINISTRADORES, PRESIDENCIA Y GERENCIA
        if ($_SESSION['id_rol'] >= 1 && $_SESSION['id_rol'] <= 3) {
            $IdProductos = (empty($_GET['idproducto'])) ? NULL : $_GET['idproducto']; // ID UNICO DE USUARIOS
            // EVITAR INGRESO VACIO {ACCIDENTAL O CON INTENCION}
            if (empty($IdProductos)) {
                ...
                $consulta = $Gestiones->DesactivarProductosCrediAgil($conectarsistema, $IdProductos);
                // ENVIANDO RESPUESTA DE ACCION A MODELO
                echo json_encode($consulta);
            }
            $conectarsistema->close(); // CERRANDO CONEXION
    ...
    case "expirar-productos-CrediAgil":
        // VISTA VALIDA PARA ADMINISTRADORES, PRESIDENCIA Y GERENCIA
        if ($_SESSION['id_rol'] >= 1 && $_SESSION['id_rol'] <= 3) {
            $IdProductos = (empty($_GET['idproducto'])) ? NULL : $_GET['idproducto']; // ID UNICO DE USUARIOS
            // EVITAR INGRESO VACIO {ACCIDENTAL O CON INTENCION}
            if (empty($IdProductos)) {
                ...
                $consulta = $Gestiones->ExpirarProductosCrediAgil($conectarsistema, $IdProductos);
                // ENVIANDO RESPUESTA DE ACCION A MODELO
                echo json_encode($consulta);
            }
            $conectarsistema->close(); // CERRANDO CONEXION
    ...
    case "consulta-productos-especifica-CrediAgil":
        ...
        if ($_SESSION['id_rol'] == 1) {
            ...
            $consulta = $Gestiones->ConsultaGeneralProductosCrediAgil($conectarsistema, $IdProductos);
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta1 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema1, $IdUsuarios);
            require('../vista/Administradores/consulta-especifica-productos-CrediAgil.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "consulta-productos-especifica-CrediAgil-presidencia":
        ...
        if ($_SESSION['id_rol'] == 2) {
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta1 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema1, $IdUsuarios);
            require('../vista/Presidencia/consulta-especifica-productos-CrediAgil-presidencia.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "consulta-productos-especifica-CrediAgil-gerencia":
        ...
        if ($_SESSION['id_rol'] == 3) {
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta1 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema1, $IdUsuarios);
            require('../vista/Gerencia/consulta-especifica-productos-CrediAgil-gerencia.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "consulta-productos-especifica-CrediAgil-atencion-clientes":
        ...
        if ($_SESSION['id_rol'] == 4) {
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta1 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema1, $IdUsuarios);
            require('../vista/AtencionClientes/consulta-especifica-productos-CrediAgil-atencion-clientes.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "envio-datos-modificar-productos-CrediAgil":
    ...
    case "envio-datos-asignacion-prestamos-CrediAgil-clientes":
        // VISTA VALIDA PARA ADMINISTRADORES, PRESIDENCIA, GERENCIA, ATENCION AL CLIENTE
        // LOS EMPLEADOS DE [ATENCION AL CLIENTE] SON QUIENES PRINCIPALMENTE GESTIONARAN TODAS LAS NUEVAS SOLICITUDES CREDITICIAS -> EN CASOS MUY PUNTUALES PUEDE HACERLO LOS OTROS ROLES DE USUARIOS EXCLUYENDO A [CLIENTES]
        if ($_SESSION['id_rol'] >= 1 && $_SESSION['id_rol'] <= 4) {
    ...
    */
    ...
    case "sistema-pagos-creditos-CrediAgil-clientes":
        ...
        if ($_SESSION['id_rol'] == 1) {
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta2 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema3, $IdUsuarioSistema);
            require('../vista/Administradores/consulta-cuotas-creditos-sistema-pagos-CrediAgil.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "sistema-pagos-creditos-CrediAgil-clientes-atencion-al-cliente":
        ...
        if ($_SESSION['id_rol'] == 4) {
            ...
            // LISTADO RECORTADO DE NOTIFICACIONES -> BARRA DE HERRAMIENTAS PLATAFORMA
            $consulta2 = $Gestiones->MostrarListadoNotificacionesRecortadaRecibidasUsuarios($conectarsistema3, $IdUsuarioSistema);
            require('../vista/AtencionClientes/consulta-cuotas-creditos-sistema-pagos-CrediAgil-atencion-al-cliente.php');
            $conectarsistema->close(); // CERRANDO CONEXION PRINCIPAL
            // -> CIERRE DE CONEXIONES AUXILIARES
            $conectarsistema1->close(); // CERRANDO CONEXION AUXILIAR 1
            $conectarsistema2->close(); // CERRANDO CONEXION AUXILIAR 2
    ...
    case "redirecciones-gestiones-cuentas-clientes":
        ...
        if ($_SESSION['id_rol'] == 1) {
            header('location:cGestionesCrediAgil.php?CrediAgilgestion=consulta-transacciones-cuentas-clientes');
            // REDIRECCIONAR INICIO PORTAL PRESIDENCIA [USUARIOS LOGUEADOS]
        } else if ($_SESSION['id_rol'] == 2) {
            header('location:cGestionesCrediAgil.php?CrediAgilgestion=consulta-transacciones-cuentas-clientes-presidencia');
            // REDIRECCIONAR INICIO PORTAL GERENCIA [USUARIOS LOGUEADOS]
        } else if ($_SESSION['id_rol'] == 3) {
            header('location:cGestionesCrediAgil.php?CrediAgilgestion=consulta-transacciones-cuentas-clientes-gerencia');
            // REDIRECCIONAR INICIO PORTAL ATENCION AL CLIENTE [USUARIOS LOGUEADOS]
        } else if ($_SESSION['id_rol'] == 4) {
            header('location:cGestionesCrediAgil.php?CrediAgilgestion=consulta-transacciones-cuentas-clientes-atencion-al-cliente');
            // REDIRECCIONAR INICIO PORTAL CLIENTES [USUARIOS LOGUEADOS]
        } else if ($_SESSION['id_rol'] == 5) {
            header('location:cGestionesCrediAgil.php?CrediAgilgestion=consulta-transacciones-cuentas-clientes-portalcliente');
        } else {
    ...
    default:
        ...
        if ($_SESSION['id_rol'] >= 1 && $_SESSION['id_rol'] <= 5) {
            header('location:cGestionesCrediAgil.php?CrediAgilgestion=error-404');
            // REDIRECCIONAR INICIO PORTAL PRESIDENCIA [USUARIOS LOGUEADOS]
        } else {
            // SI LOS USUARIOS NO ESTAN LOGUEADOS, E INGRESAN PARAMETROS NO [O BIEN] EXISTENTES EN EL SISTEMA, SIMPLEMENTE REDIRIGIR A FORMULARIO DE INICIO DE SESION
            header('location:cIniciosSesionesUsuarios.php?CrediAgil=iniciarsesion');
